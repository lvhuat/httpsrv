	...
	return func(httpCtx *gin.Context) {
		...
		defer func() {
			...
			if wrapper.enableStat {
				stat.Stat(httpCtx, http.StatusOK, cerr, time.Now().Sub(since))
			}
		}()

		// 过载保护
		if wrapper.overloadChecker != nil {
			cerr = wrapper.overloadChecker.Check(httpCtx)
			if cerr != nil {
				return
			}
		}
...
func (wrapper *Wrapper) wrapperNoWrapperStatus(f func(ginCtx *gin.Context) (NoWrapperResponse, int)) func(httpCtx *gin.Context) {
	return func(httpCtx *gin.Context) {
		...
		defer func() {
			...
			if wrapper.enableStat {
				stat.Stat(httpCtx, status, cerr, time.Now().Sub(since))
			}
		}()

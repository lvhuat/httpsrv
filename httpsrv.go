...
var (
	// DefaultOverloadLimit  默认过载保护
	DefaultOverloadLimit int32 = 20000
)
...
func (option *Option) useDefault() {
	...
	if option.OverloadChecker == nil {
		if option.OverloadLimit <= 0 {
			option.OverloadChecker = &overloadChecker{
				LimitCnt: DefaultOverloadLimit,
				Service:  option.Prefix,
			}
		} else {
			option.OverloadChecker = &overloadChecker{
				LimitCnt: option.OverloadLimit,
				Service:  option.Prefix,
			}
		}
	}
...
func New(option *Option) *Wrapper {
	...
	w := &Wrapper{
		...
		overloadChecker: option.OverloadChecker,
...
func (wrapper *Wrapper) HandleStat() {
	...
	wrapper.Get("/debug/httpstat/delay", stat.StatDelay)
	wrapper.Get("/debug/httpstat/result", stat.StatResult)
}
